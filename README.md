...
>
> **UI 已切换到 PySide6 (Qt6)**，享受真·半透明窗口 + QPainter 抗锯齿，猫与气泡边缘丝滑无毛边；形象素材升级为 512px 高清版并做了高 DPI 适配，在 125% / 150% 缩放的屏幕上同样锐利。妮子有**坐姿**与**睡姿**两套形象：暂停时蜷起来睡下、继续时再坐起来，中间带一段自然的过渡。
...
- ❄️ **暂停一眼可见** — 暂停时贴纸转为琥珀色并保持清晰（猫本体不变色），妮子本人也**蜷成一团睡下**，远远扫一眼就知道现在停着
- 😴 **姿态过渡** — 坐姿 ↔ 睡姿之间是带缓动的交叉淡化过渡（340 ms，伴随轻微下沉与缩放），不是生硬的瞬间换图
- 🕐 **暂停时长正计时** — 暂停后贴纸上的数字从「还剩多久」切换为「**已停多久**」的正计时，菜单项也会写成「继续计时（已停 32 分钟）」，停太久心里有数
- ⏰ **暂停超时自动继续** — 停太久（默认 30 分钟，可关）自动恢复计时，避免忘了取消暂停就一直停在那儿
- 💬 **到点气泡提醒** — 计时归零弹出奶米贴纸风气泡，尾巴始终指向小猫本体；话术支持 7 种风格：**默认**（原版 10 条）/ **傲娇 / 温柔 / 呆萌 / 冷漠 / 可爱**（各 20 条）/**随机**（全池抽），可在设置里切换，每次不与上一条重复；气泡上并排「**知道了**」和「**N 分钟后**」两颗按钮，后者只把这一轮往后推，不改你设的间隔
- 🌙 **免打扰时段** — 可设一个时段（默认 22:00–08:00，支持跨夜），该时段内到点只静默重置、不弹气泡，计时照常走
- ⚙️ **自定义间隔** — 提醒间隔支持分钟（1–600）与秒（5–36000）两种单位，切换单位时已输入的数字保持不变，保存到本地 `config.json`，重启不丢；设置里另有 15 / 30 / 45 / 60 分钟四个快捷值，点一下即填入
- 📏 **妮子大小三档** — 小（88）/ 中（110）/ 大（132）可选，窗口尺寸与素材按比例同步推导，**换档时保持窗口中心不动**，妮子不会跳位
...
- `config.json` 会在 exe 同目录自动生成，记录间隔 / 窗口位置 / 妮子大小 / 贴纸显示 / 话术风格 / 免打扰时段 / 暂停超时 / 开机自启等偏好
...
| 提醒弹出后点「N 分钟后」 | 只把**这一轮**往后推 N 分钟（默认 5），不改你设置的提醒间隔 |
| 设置窗间隔快捷按钮 | 15 / 30 / 45 / 60 分钟，点一下填入输入框（仍需点「保存」才生效） |
| 设置窗「妮子大小」 | 小（88）/ 中（110）/ 大（132）三档；保存后即时生效，换档时窗口中心不动 |
| 设置窗「暂停超时自动继续」 | 勾选 + 分钟数（默认 30）。暂停超过该时长自动恢复计时；不勾 = 永不自动恢复 |
| 设置窗「免打扰时段」 | 勾选 + 起止时间（默认 22:00–08:00，支持跨夜）。时段内到点不弹气泡，计时照走 |
...
sitreminder/
  ...
  quiet.py                  # 免打扰时段判断（HH:MM 解析、支持跨夜、左闭右开边界）
...
assets/
  nizi_clean_qt.png         # 主用形象·坐姿：512px 高清、边缘去色晕（高 DPI 适配）
  nizi_sleep_qt.png         # 主用形象·睡姿：暂停时显示；底边与水平中心已和坐姿对齐（切换不跳位）
...
tests/
  test_core.py              # 核心逻辑单元测试（框架无关，53 项）
...
> 打包时带入 `nizi_clean_qt.png`、`nizi_sleep_qt.png`、`nizi_clean.png`、`icon.ico` 四个资源；
...
| `mascot_size` | 妮子大小档位（像素边长）：`88` / `110` / `132`（默认 `110`）。手改成其它数值会**吸附到最近档位**（如 `100` → `110`） |
| `pause_auto_resume_seconds` | 暂停多久后自动恢复计时（秒），默认 `1800`（30 分钟）；`0` = 关闭该行为，暂停完全由你手动解除 |
| `snooze_seconds` | 气泡上「N 分钟后」的推后时长（秒），默认 `300`（5 分钟）。**只影响这一轮**，不改 `interval_seconds` |
| `quiet_enabled` | 免打扰时段开关（默认 `true`） |
| `quiet_start` / `quiet_end` | 免打扰起止时间，`"HH:MM"` 格式，默认 `22:00` / `08:00`。**起 > 止 视为跨夜**；两者相同或格式非法则视为不生效（否则等于整天免打扰） |
...
- **姿态过渡不靠形变**：坐姿 ↔ 睡姿采用「交叉淡化 + 反向位移 + 缩放」。两张素材轮廓本来就没有对应点，逐帧形变必然别扭，而淡化不要求任何对应关系。不透明度按 `(1-e)^2.2` 与其互补值分配 —— **两层之和恒为 1**，否则过渡到一半整只猫会忽明忽暗；退场者下沉并缩到 0.94、入场者从上方落下并放回 1.0，观感上「旧的退到后面去」，重叠期不会糊成一团
- **免打扰是左闭右开**：含起点、不含终点；起止时间填成一样即视为不生效（否则等于整天免打扰）；被拦下的那一轮必须静默重置，否则到期状态恒为真、250 ms 空转
- **动画**：气泡使用 easeOut 缓动的生长动画；倒计时贴纸用不透明度补间在「淡显 ↔ 清晰」之间平滑过渡，进出自然不生硬
- **防出屏**：气泡、「关闭…」确认框与设置窗在屏幕边缘都会自动收拢（右 / 下放不下时翻到另一侧或收进可用区），气泡尾巴始终指向小猫。设置窗带系统标题栏，**定位必须按「外框」而不是客户区算**（125% 缩放下二者差 30 px），否则妮子贴着屏幕底部时「保存」那一行会被切掉 —— 做法是先 `show()` 再按真实 `frameGeometry()` 收敛一次
